...
export default function ProductCard({ producto, onAdd }: ProductCardProps) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden flex flex-col transform transition-all duration-300 ease-out hover:-translate-y-1.5 hover:shadow-xl hover:border-gray-200 group">
      {/* Contenedor de Imagen con efecto Swap de Hover */}
          ...
          <>
            ...
            <img
              ...
              className={`w-full h-full object-cover transition-opacity duration-500 ease-out ${
                producto.imagen2 ? 'opacity-100 group-hover:opacity-0' : 'group-hover:scale-105'
              }`}
            />
            
            {/* Segunda Imagen (Aparece en Hover si existe) */}
            {producto.imagen2 && (
              <img
                src={producto.imagen2}
                alt={`${producto.nombre} vista alternativa`}
                className="absolute inset-0 w-full h-full object-cover opacity-0 group-hover:opacity-100 transition-all duration-500 ease-out scale-100 group-hover:scale-105"
              />
            )}
          </>
        )}
